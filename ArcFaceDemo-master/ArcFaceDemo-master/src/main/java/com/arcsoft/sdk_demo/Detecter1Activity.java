...
import com.arcsoft.sdk_demo.util.DBManager;
import com.arcsoft.sdk_demo.util.FaceRegister;
import com.arcsoft.sdk_demo.util.SerialPortInterface;
...
public class Detecter1Activity extends Activity implements OnCameraListener, View.OnTouchListener, Camera.AutoFocusCallback, View.OnClickListener {
	...
	class FRAbsLoop extends AbsLoop {
		...
		List<FaceDB.FaceRegist> mResgist = ((Application)Detecter1Activity.this.getApplicationContext()).mFaceDB.mRegister;
				...
				if (max > 0.6f) {
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							...
							if(dbManager.isFaceRegisterExist(mNameShow))
							{

									Toast.makeText(getApplicationContext(), "该用户已经存入物品!", Toast.LENGTH_SHORT).show();
							...
							else
							{
								int count = 0;
								for (int i =1;i<5;i++) {
									if(dbManager.isFaceRegisterExistByboxID(i))
									{
										count=i;
									}
									else
								...
								if(count==4)
								{
									Toast.makeText(getApplicationContext(), "所有柜子已经存满", Toast.LENGTH_SHORT).show();
								}
								else {
									faceRegister = new FaceRegister(mNameShow, count+1);
									Toast.makeText(getApplicationContext(), ""+(count+1)+"号柜子已打开", Toast.LENGTH_SHORT).show();
									spi.sendOpenLock(1,count+1);
									dbManager.addRegister(faceRegister);
								}
					...
					Detecter1Activity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							mTextView.setAlpha(1.0f);
							mTextView1.setVisibility(View.VISIBLE);
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        spi = new SerialPortInterface(this,"/dev/ttyS4",38400);
